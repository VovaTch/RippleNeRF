...
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400):
    ...
    plt.figure()
    plt.imshow(img)
    if not os.path.isdir("novel_views"):
        os.makedirs("novel_views")
    plt.savefig(
...
def train(
    ...
    training_loss = []
    for idx in tqdm(range(nb_epochs), desc="Training epochs..."):
        for batch in tqdm(data_loader, desc=f"Training batches for epoch {idx + 1}"):
...
if __name__ == "__main__":
    ...
    training_dataset = torch.from_numpy(
        np.load("data/truck_data/training_data.pkl", allow_pickle=True)
    )
    testing_dataset = torch.from_numpy(
        np.load("data/truck_data/testing_data.pkl", allow_pickle=True)
    )
